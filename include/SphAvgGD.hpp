...
class SphericalAverageGD {
 public:
  SphericalAverageGD()
      : N(0), M(0), converged(false), w(), p(), q(), v(), g() {}

  SphericalAverageGD(size_t num_point, size_t num_feature,
                     T* unnormalized_vectors)
      : N(num_point),
        M(num_feature),
        converged(false),
        w(),
  ...
  bool update(void) {
    ...
    */
    if (norm_g >= 8 * std::numeric_limits<T>::epsilon()
  ...
  const std::vector<T>& get_weights(void) { return v; }
