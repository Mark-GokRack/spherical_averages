...
class SphericalAverageLBFGS_align {
 public:
  ...
  SphericalAverageLBFGS_align(size_t num_point, size_t num_feature,
                              T* unnormalized_vectors, size_t num_memory = 8)
      : N(num_point),
        M(num_feature),
        K(num_memory),
        converged(false),
  ...
  void initialize(size_t num_point, size_t num_feature, T* unnormalized_vectors,
    ...
    std::copy_n(unnormalized_vectors, N * M, p.begin());
  ...
  const AlignedVector<T, 64> get_weights(void) { return v; }
...
#pragma clang loop vectorize(enable) unroll(enable)
  ...
  inline void add_product_c(size_t len, T a, const T* __restrict x,
                            T* __restrict y) {
...
#pragma clang loop vectorize(enable) unroll(enable)
  ...
  inline void project_vector_to_plane(size_t len, const T* __restrict x,
                                      T* __restrict y) {
